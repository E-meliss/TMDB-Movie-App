...
<template>
  <div
    ...
    class="absolute z-50 w-64 mt-4 text-sm bg-gray-800 rounded text-mywhite search-list-container"
  >
    <ul v-if="searchResults.length">
      <li
        class="border-b border-gray-600 dark:border-gray-700 search-list-item"
        v-for="result in sliceArray(searchResults, 0, 7)"
        :key="result"
      >
        <div
          v-if="result.media_type == 'movie'"
          class="flex items-center px-3 py-3 transition duration-150 ease-in-out search-item"
        >
          <ImageOrPlaceholder
            :result="result"
            my_key="poster_path"
